...
contract GANNodeVault is OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable, IErrors
{
    ...
    function withdrawNodeFor(address user,uint120 amount) external nonReentrant validAmount(amount) onlyOrderbookHandler
    {
        ...
        {
            uint tokenIndex = _tokenId.length - 1;
            _ganNode.safeTransferFrom(address(this), user, _tokenId[tokenIndex]);
            _tokenId.pop(); 
        }
        
        emit WithdrawFor(user, amount, block.timestamp);
    }

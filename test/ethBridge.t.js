...
describe("Bridge:", () => {
  ...
  before(async () => {
    ...
    gpuTokenProxy = await upgrades.deployProxy(gpuTokenFactory, [ethers.parseEther("1000000")], {
      initializer: "initialize",
      from: owner.address,
    });

    ethBridgeFactory = await ethers.getContractFactory("GpuEthBridge");
    ethBridgeProxy = await upgrades.deployProxy(ethBridgeFactory, [gpuTokenProxy.target,relayer1.address], {
      initializer: "initialize",
      from: owner.address,
    });

    GanBridgeFactory = await ethers.getContractFactory("GanChainBridge");
    ganBridgeProxy = await upgrades.deployProxy(GanBridgeFactory, [relayer1.address], {
      initializer: "initialize",
      from: owner.address,
    });
  });
  ...
  describe("check relayers", () => {
    it("Should set relayer2 and 3", async () => {
      await expect(await ganBridgeProxy._isRelayer(relayer2.address)).to.be.equals(false);
      await expect(await ethBridgeProxy._isRelayer(relayer3.address)).to.be.equals(false);
      await ganBridgeProxy.connect(owner).setRelayers([relayer2.address,relayer3.address,],[true,true]);
      await ethBridgeProxy.connect(owner).setRelayers([relayer2.address,relayer3.address,],[true,true]);
      await expect(await ganBridgeProxy._isRelayer(relayer2.address)).to.be.equals(true);
      await expect(await ethBridgeProxy._isRelayer(relayer3.address)).to.be.equals(true);
      await expect(await ganBridgeProxy._isRelayer(relayer1.address)).to.be.equals(true);
    });
  });
  ...
  describe("Make the swap from GAN chain -> Ethereum", () => {
    ...
    it("Should release GPU to user-1 on Ethereum", async () => {
      await gpuTokenProxy.connect(owner).transfer(ethBridgeProxy.target, ethers.parseEther("500000"));
      await expect(await gpuTokenProxy.balanceOf(ethBridgeProxy.target)).to.equal(ethers.parseEther("500000"));
      await expect(await gpuTokenProxy.balanceOf(user1)).to.equal(ethers.parseEther("0"));
      await ethBridgeProxy.connect(relayer1).releaseGpu(ethers.parseEther("10"),user1);
      await expect(await gpuTokenProxy.balanceOf(user1)).to.equal(ethers.parseEther("10"));
      await expect(await ethBridgeProxy._totalReleasedAmount(user1.address)).to.be.equals(ethers.parseEther("10"));
      console.log("TotalReleasedAmount: "+await ethBridgeProxy._totalReleasedAmount(user1.address));
      console.log("TotalReleasedAmount: "+await ethBridgeProxy._totalReleasedAmount(user2.address));
      console.log("releaseID: "+await ethBridgeProxy._releaseId());
      await expect(await ethBridgeProxy._releasedUser(0)).to.be.equals(user1.address);
    });
  });

  describe("Make the swap from Ethereum -> GAN chain ", () => {
    it("Should let users lock GPU on Ethereum", async () => {
      await expect(await gpuTokenProxy.balanceOf(ethBridgeProxy.target)).to.equal(ethers.parseEther("499990"));
      await gpuTokenProxy.connect(owner).transfer(user2, ethers.parseEther("69"));
      await gpuTokenProxy.connect(user2).approve(ethBridgeProxy.target, ethers.parseEther("69"));
      await ethBridgeProxy.connect(user2).lockGpu(ethers.parseEther("69"));
      await expect(await gpuTokenProxy.balanceOf(ethBridgeProxy.target)).to.equal(ethers.parseEther("500059"));
      await expect(await ethBridgeProxy._totalLockedAmount(user2.address)).to.be.equals(ethers.parseEther("69"));
      await expect(await ethBridgeProxy._lockedUser(0)).to.be.equals(user2.address);
    });

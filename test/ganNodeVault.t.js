...
describe("GANNodeVault", () => {
  ...
  describe("GanNodeVault::", () => {
    ...
    await expect(ganNodeVaultProxy.connect(user1).depositNode(2,[1,2,3])).to
    .be.revertedWithCustomError(ganNodeVaultProxy,"incorrectArraySize");
    console.log("approval status before:"+await ganNode.isApprovedForAll(user1.address,ganNodeVaultProxy.target));
    await expect(ganNodeVaultProxy.connect(user1).depositNode(2,[1,2])).to.
    be.revertedWithCustomError(ganNodeVaultProxy,"contractNotApproved");
    ...
    await expect(ganNodeVaultProxy.connect(owner).unlock(user2,3)).to.
    be.reverted;
    ...
    await expect(ganNodeVaultProxy.connect(user2).withdrawNode(4)).to.
    be.revertedWithCustomError(ganNodeVaultProxy,"InsufficientUnlockedBalance");
    await ganNodeVaultProxy.connect(user2).withdrawNode(2);
    await expect(ganNodeVaultProxy.connect(user2).withdrawNode(1)).to.
    be.revertedWithCustomError(ganNodeVaultProxy,"inSufficientBalanceInContract");
    console.log("user2 Nft balance:"+await ganNode.balanceOf(user2.address));
    });
  });

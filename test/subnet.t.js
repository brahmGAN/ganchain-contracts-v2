...
describe("Subnet", () => {
  ...
  before(async () => {
    ...
    subnetProxy = await upgrades.deployProxy(
      subnetFactory,
      [updater.address],
      { 
  ...
  describe("Owner sets all the user related details",()=>{
    it("Should set all the details of the users", async()=>{
      //setKingRewards
      await subnetProxy.connect(owner).setKingRewards([king1,king2],[ethers.parseEther("10"),ethers.parseEther("20")]);
      //setQueenRewards
      await subnetProxy.connect(owner).setQueenRewards([queen1,queen2],[ethers.parseEther("30"),ethers.parseEther("40")]);
      //setBatchUserMaxVotes
      await subnetProxy.connect(updater).setBatchUserMaxVotes([queen1,queen2],[5,10]);

      await expect(await subnetProxy._pendingRewards(queen2)).to.be.equals(ethers.parseEther("40"));
  ...
  describe("Users should interact with all user callable functions", ()=>{
    ...
    it("Should not create multiple subnets",async()=>{
      await expect(subnetProxy.connect(king1).createSubnet()).to.be.
      revertedWithCustomError(subnetProxy,"cannotCreateMultipleSubnets");
    });
    ...
    it("Should claim rewards",async()=>{
      await owner.sendTransaction({ to: await subnetProxy.getAddress(), value: ethers.parseEther("100") });
      await expect(await subnetProxy.connect(queen2).claimRewards(ethers.parseEther("10"))).to.
      emit(subnetProxy,"claimedRewards").
    ...
    it("Should cast vote for multiple subnets",async()=>{
      ...
      await expect(await subnetProxy._userVotesToSubnet(queen2.address,2)).to.be.equals(4);
    });
    it("Should un-cast vote for multiple subnets",async()=>{
      await subnetProxy.connect(queen2).unCastVotes([1,2],[1,2],3);
      await expect(await subnetProxy._userVotesToSubnet(queen2.address,2)).to.be.equals(2); 
    });
  });

  describe("Call all the setter fucntions",()=>{
    it("should authorizedRewardSender()",async()=>{
      await expect(await subnetProxy._pendingRewards(king1)).to.be.equals(ethers.parseEther("10"));
      await subnetProxy.connect(owner).authorizedRewardSender([king1,king2],[ethers.parseEther("10"),ethers.parseEther("20")]);
      await expect(await subnetProxy._pendingRewards(king1)).to.be.equals(ethers.parseEther("0"));
    });
    ...
    it("should setBatchUserCastedVotes",async()=>{
      ...
      await subnetProxy.connect(updater).setBatchUserCastedVotes([king1,king2],[5,6]);
    ...
    it("should setBatchUserVotesToSubnet",async()=>{
      await expect(await subnetProxy._userVotesToSubnet(queen2.address,2)).to.be.equals(2); 
      await subnetProxy.connect(updater).setBatchUserVotesToSubnet([king1,queen2],[1,77],[1,2]);
      await expect(await subnetProxy._userVotesToSubnet(queen2.address,2)).to.be.equals(77); 
    });
    ...
    it("should setUserVotesToSubnet",async()=>{
      await expect(await subnetProxy._userVotesToSubnet(king1.address,1)).to.be.equals(1); 
      await expect(subnetProxy.connect(updater).setUserVotesToSubnet(king1,69,1)).to.be.revertedWith("You are not the authorized updater");
      await subnetProxy.connect(newUpdater).setUserVotesToSubnet(king1,69,1)
      await expect(await subnetProxy._userVotesToSubnet(king1.address,1)).to.be.equals(69); 
    });
  });
});
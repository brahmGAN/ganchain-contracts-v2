...
describe("Subnet", () => {
  ...
  let seller; 
  let buyer; 
  const uri = "https://lavender-puny-mosquito-504.mypinata.cloud/ipfs/QmR3udFV7BDMVt1pUDpsg4Y7jdLFCdpC5VwV7BkT9gMKYJ";
  before(async () => {
    [
      owner,
      seller,
    ...
    nodeProxy = await upgrades.deployProxy(
      nodeFactory,
      [],
      { 
  ...
  describe("Marketplace", () => {
    it("Should mint node and seller should put a sell order and buyer completes the order", async () => {
      ...
      await nodeProxy.connect(owner).setTierPrice([ethers.parseEther("1"),ethers.parseEther("2"),ethers.parseEther("3"),ethers.parseEther("4")]);
      ...
      await nodeProxy.connect(buyer).buyNodes(1,0,[1],{value: ethers.parseEther("4")});
